...
//     //assuming x is on the side, y axis pointing you, I guess I could have done better to have
//     the axes in reverse order but ok
...
//     but will never be processed further, so just ignore this return 0;
// }
...
__device__ real_t buf_at(real_t *buffer, int_t i, int_t j, int_t k) {
    ...
    if(i < 0 || i >= PML_WIDTH)
        return 0; // out of bounds, can happen

    // if(i == PML_WIDTH/2 && j == d_Ny/2 && k == d_Nz/2)
    //     printf("then, reading %lf at index %d\n",  buffer[i * (d_Ny * d_Nz) + j * d_Nz + k]);

    return buffer[i * (d_Ny * d_Nz) + per(j, d_Ny) * d_Nz
                  + per(k, d_Nz)]; // access might need to be periodic
}
...
void domain_save(int_t step) {
    ...
    // {
    //     // cudaErrorCheck(cudaMemcpy(&saved_buffer[Nx*i], &(d_buffer[(Nx/2+PADDING) *
    //     (Ny+2*PADDING) * (Nz+2*PADDING) + (i+PADDING) * ((Nz + 2*PADDING)) + (0+PADDING)]),
    //     Nz*sizeof(real_t), cudaMemcpyDeviceToHost));
    // }
    cudaErrorCheck(
        cudaMemcpy(saved_buffer,
                   d_buffer,
                   sizeof(real_t) * (Nx + 2 * PADDING) * (Ny + 2 * PADDING) * (Nz + 2 * PADDING),
    ...
    for(int j = 0; j < Ny; j++) // take some slice
    {
        for(int_t i = 0; i < Nx; i++) {
            int w =
                fwrite(&saved_buffer[(i + PADDING) * ((Ny + 2 * PADDING) * (Nz + 2 * PADDING))
                                     + (j + PADDING) * ((Nz + 2 * PADDING)) + (Nz / 2 + PADDING)],
                       sizeof(real_t),
                       1,
...
void domain_initialize() // at this point I can load an optional starting state. (I would need two
                         // steps actually)
{
    // alloc cpu memory for saving image
    // I take a YZ portion of the plane
    saved_buffer = (real_t *) calloc((Nx + 2 * PADDING) * (Ny + 2 * PADDING) * (Nz + 2 * PADDING),
                                     sizeof(real_t));
    if(!saved_buffer) {
    ...
    cudaErrorCheck(cudaMalloc(&d_phi_x_prv, border_size));
    cudaErrorCheck(cudaMalloc(&d_phi_y_prv, border_size));
    cudaErrorCheck(cudaMalloc(&d_phi_z_prv, border_size));

    cudaErrorCheck(cudaMalloc(&d_psi_x_prv, border_size));
    cudaErrorCheck(cudaMalloc(&d_psi_y_prv, border_size));
    cudaErrorCheck(cudaMalloc(&d_psi_z_prv, border_size));
    ...
    cudaErrorCheck(cudaMemset(d_phi_x, 0, border_size));
    cudaErrorCheck(cudaMemset(d_phi_y, 0, border_size));
    cudaErrorCheck(cudaMemset(d_phi_z, 0, border_size));

    cudaErrorCheck(cudaMemset(d_psi_x, 0, border_size));
    cudaErrorCheck(cudaMemset(d_psi_y, 0, border_size));
    cudaErrorCheck(cudaMemset(d_psi_z, 0, border_size));
...
// I could split phi and psi because they dont touch exactly the same indices (at border), so less
// warping
__global__ void aux_variable_step(const real_t *d_buffer,
                                  real_t *d_phi_x_prv,
                                  real_t *d_phi_y_prv,
                                  real_t *d_phi_z_prv,
                                  real_t *d_psi_x_prv,
    ...
    int i = blockIdx.x * blockDim.x + threadIdx.x
          + PML_SHIFT; // local coord passed, turned to global coordinates
    int j = blockIdx.y * blockDim.y + threadIdx.y;
    int k = blockIdx.z * blockDim.z + threadIdx.z;

    if(i >= PML_SHIFT + PML_WIDTH || j >= d_Ny || k >= d_Nz) {
        // printf("pre exit aux var func: %d %d %d\nmax &%d %d %d\n",i,j,k, PML_SHIFT + PML_WIDTH,
        // d_Nz, d_Ny);
        return;
    }
    ...
    //     printf("phix: %lf -> %lf\n", buf_at(d_phi_x_prv, i,j,k), next_phi_x);
    // }
...
__global__ void time_step(real_t *d_buffer_prv,
    ...
    //     // double psi_y_val = buf_at(d_psi_y_prv, i, j+1, k);
    //     // if (psi_y_val != 0) printf("buf_at(d_psi_y_prv, %d, %d, %d) = %f\n", i, j+1, k,
    //     psi_y_val);

    //     // double phi_y_val = buf_at(d_phi_y_prv, i, j-1, k);
    //     // if (phi_y_val != 0) printf("buf_at(d_phi_y_prv, %d, %d, %d) = %f\n", i, j-1, k,
    //     phi_y_val);

    //     // double psi_z_val = buf_at(d_psi_z_prv, i, j, k+1);
    //     // if (psi_z_val != 0) printf("buf_at(d_psi_z_prv, %d, %d, %d) = %f\n", i, j, k+1,
    //     psi_z_val);

    //     // double phi_z_val = buf_at(d_phi_z_prv, i, j, k-1);
    //     // if (phi_z_val != 0) printf("buf_at(d_phi_z_prv, %d, %d, %d) = %f\n", i, j, k-1,
    //     phi_z_val);
    //     }
...
void simulation_loop(void) {
    ...
    for(int_t iteration = 0; iteration < max_iteration; iteration++) {
        ...
        dim3 gridSize((Nx + block_x - 1) / block_x,
                      (Ny + block_y - 1) / block_y,
                      (Nz + block_z - 1) / block_z);
        dim3 pml_gridSize((PML_WIDTH + block_x - 1) / block_x,
                          (Ny + block_y - 1) / block_y,
                          (Nz + block_z - 1) / block_z);
        ...
        // time_step<<<gridSize, blockSize>>>(d_buffer_prv, d_buffer_nxt, d_buffer, d_phi_x_prv,
